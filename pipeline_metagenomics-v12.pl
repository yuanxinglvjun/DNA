...
sub profile_collection{
	...
	#foreach my $nn(qw/taxonomy nr_kingdom nr_phylum nr_class nr_order nr_family nr_genus nr_species/){
...
=cut
	...
	$queue->set_work_dir($workdir);
        $queue->addcommond("mkdir -p $outdir/composition/krona-tax");
        $queue->addcommond("cd $outdir/composition/krona-tax");
        $queue->addcommond("biom convert -i $outdir/profile_collection/gene_taxonomy_table.biom -o $outdir/composition/krona-tax/gene_taxonomy_table.txt --to-tsv --header-key taxonomy --output-metadata-id 'Consensus Lineage'"); 
        $queue->addcommond("$script/use_krona.py $outdir/profile_collection/gene_taxonomy_table.txt");
        $queue->addcommond("cd -");
...
sub comparison{
	...
	foreach my $nn(qw/nr_kingdom nr_phylum nr_class nr_order nr_family nr_genus nr_species eggnog kegg cazy_family cazy_class ardb_class ardb_type card gene/){
		...
		foreach my $dist(qw/bray_curtis euclidean hellinger/){
			$queue->addcommond("$script/plot-heatmap-dis.pl -i $outdir/comparison/distance/$nn/$dist\_$nn\_table.txt -o $outdir/comparison/distance/$nn/$dist\_$nn\_heatmap -lw 0.1:1:0.1:7.5 -lh 1:0.1:7.5:1.8 -h 6 -slas 2 -col gray-lightyellow-darkorange");
		}
	}
	$queue->run();
	$queue->wait();
	...
	foreach my $nn(qw/nr_kingdom nr_phylum nr_class nr_order nr_family nr_genus nr_species eggnog kegg cazy_family cazy_class ardb_class ardb_type card gene/){
		foreach my $std(sort keys %group_info){
			...
			if(scalar keys %{$group_info{$std}} == 2){
				$queue->addcommond("group_significance.py -i $outdir/profile_collection/group/$nn/$std/data.biom -m $outdir/group_info/$std.info -c $std -s mann_whitney_u -o $outdir/difference/classic/$nn/$std/difference.xls &> $outdir/difference/classic/$nn/$std/log.txt");
			}else{
...
sub result_collection{
	...
	$queue->addcommond("mkdir -p $outdir/Result/01.Rawdata_Stats");
	$queue->addcommond("cp $outdir/fastq_QC/rawdata.stat.xls  $outdir/Result/01.Rawdata_Stats/");
	$queue->addcommond("cp $outdir/fastq_QC/cleandata.stat.xls $outdir/Result/01.Rawdata_Stats/cleandata.nohost.stat.xls");
	$queue->addcommond("cp $outdir/fastq_QC/*fastqc.zip  $outdir/Result/01.Rawdata_Stats/");
	$queue->addcommond("mkdir -p $outdir/Result/02.Assembly");

	foreach my $sn(sort keys %samples){
	     $queue->addcommond("mkdir  -p $outdir/Result/02.Assembly/$sn");
	     $queue->addcommond("gzip -c $outdir/assembly/$sn/filtered_contig.fa > $outdir/Result/02.Assembly/$sn/filtered_contig.fa.gz");
	     $queue->addcommond("cp $outdir/assembly/$sn/contig_stat.txt  $outdir/Result/02.Assembly/$sn/");
	     $queue->addcommond("mkdir  -p $outdir/Result/03.GenePredict/$sn");
		 #$queue->addcommond("gzip -c $outdir/gene_predict/seq.fna > $outdir/Result/03.GenePredict/$sn/seq.fna.gz");
		 #$queue->addcommond("gzip -c $outdir/gene_predict/seq.faa > $outdir/Result/03.GenePredict/$sn/seq.faa.gz");
	     $queue->addcommond("gzip -c $outdir/gene_predict/$sn/seq_more100.fna > $outdir/Result/03.GenePredict/$sn/seq_more100.fna.gz");
	     $queue->addcommond("cp $outdir/gene_predict/$sn/seq.gff > $outdir/Result/03.GenePredict/$sn/");
	}

        $queue->addcommond("mkdir  -p $outdir/Result/04.GeneSet");
        $queue->addcommond("gzip -c $outdir/unigene_set/uniGeneSet.faa > $outdir/Result/04.GeneSet/uniGeneSet.faa.gz");
        $queue->addcommond("gzip -c $outdir/unigene_set/uniGeneSet.fna > $outdir/Result/04.GeneSet/uniGeneSet.fna.gz");

        $queue->addcommond("mkdir  -p $outdir/Result/05.GeneProfile");
	$queue->addcommond("gzip -c $outdir/gene_profile/gene_profile.TPM.txt > $outdir/Result/05.GeneProfile/gene_profile.TPM.txt.gz");
	$queue->addcommond("gzip -c $outdir/gene_profile/gene_profile.reads_number.txt > $outdir/Result/05.GeneProfile/gene_profile.reads_number.txt.gz");

	$queue->addcommond("mkdir -p $outdir/Result/06.Annotation");
	$queue->addcommond("mkdir -p $outdir/Result/06.Annotation/ardb");
	$queue->addcommond("mkdir -p $outdir/Result/06.Annotation/card");
	$queue->addcommond("mkdir -p $outdir/Result/06.Annotation/eggnog");
	$queue->addcommond("mkdir -p $outdir/Result/06.Annotation/cazy");
	$queue->addcommond("mkdir -p $outdir/Result/06.Annotation/kegg");
	$queue->addcommond("mkdir -p $outdir/Result/06.Annotation/mvirdb");
	$queue->addcommond("mkdir -p $outdir/Result/06.Annotation/nr");
	$queue->addcommond("mkdir -p $outdir/Result/06.Annotation/phi");
	$queue->addcommond("mkdir -p $outdir/Result/06.Annotation/qs");
	$queue->addcommond("mkdir -p $outdir/Result/06.Annotation/tcdb");
	$queue->addcommond("mkdir -p $outdir/Result/06.Annotation/vfdb");

	$queue->addcommond("cp -r $outdir/gene_annotation/ardb/outdir   $outdir/Result/06.Annotation/ardb/");
	$queue->addcommond("cp -r $outdir/gene_annotation/card/outdir   $outdir/Result/06.Annotation/card/");
	$queue->addcommond("cp -r $outdir/gene_annotation/eggnog/outdir   $outdir/Result/06.Annotation/eggnog/");
	$queue->addcommond("cp -r $outdir/gene_annotation/cazy/outdir   $outdir/Result/06.Annotation/cazy/");
	$queue->addcommond("cp -r $outdir/gene_annotation/kegg/outdir   $outdir/Result/06.Annotation/kegg/");
	$queue->addcommond("cp -r $outdir/gene_annotation/mvirdb/outdir   $outdir/Result/06.Annotation/mvirdb/");
	$queue->addcommond("cp -r $outdir/gene_annotation/nr/outdir   $outdir/Result/06.Annotation/nr/");
	$queue->addcommond("cp -r $outdir/gene_annotation/phi/outdir   $outdir/Result/06.Annotation/phi/");
	$queue->addcommond("cp -r $outdir/gene_annotation/qs/outdir   $outdir/Result/06.Annotation/qs/");
	$queue->addcommond("cp -r $outdir/gene_annotation/tcdb/outdir   $outdir/Result/06.Annotation/tcdb/");
	$queue->addcommond("cp -r $outdir/gene_annotation/vfdb/outdir   $outdir/Result/06.Annotation/vfdb/");
	$queue->addcommond("rm -rf $outdir/Result/06.Annotation/*/outdir/*tmp");
	$queue->addcommond("rm -rf $outdir/Result/06.Annotation/*/outdir/*r");
	$queue->addcommond("rm -rf $outdir/Result/06.Annotation/*/outdir/*/*r");
	$queue->addcommond("rm -rf $outdir/Result/06.Annotation/*/outdir/*/*blast.txt");

	$queue->addcommond("mkdir -p $outdir/Result/07.Composition");
	$queue->addcommond("mkdir -p $outdir/Result/07.Composition/Bubble");
	$queue->addcommond("mkdir -p $outdir/Result/07.Composition/Heatmap");
	$queue->addcommond("mkdir -p $outdir/Result/07.Composition/Megan");
	#$queue->addcommond("mkdir -p $outdir/Result/07.Composition/Venn");
	
	$queue->addcommond("cp -r $outdir/composition/bubble/*   $outdir/Result/07.Composition/Bubble/");
	$queue->addcommond("cp -r $outdir/composition/heatmap/*  $outdir/Result/07.Composition/Heatmap/");
	$queue->addcommond("cp -r $outdir/composition/megan/* $outdir/Result/07.Composition/Megan/");
	$queue->addcommond("rm $outdir/Result/07.Composition/*/*/*/*r");
		
	$queue->addcommond("mkdir -p $outdir/Result/08.Comparison");
	$queue->addcommond("mkdir -p $outdir/Result/08.Comparison/Adonis");
	$queue->addcommond("mkdir -p $outdir/Result/08.Comparison/Anosim");
	$queue->addcommond("mkdir -p $outdir/Result/08.Comparison/Distance");
	$queue->addcommond("mkdir -p $outdir/Result/08.Comparison/Nmds");
	$queue->addcommond("mkdir -p $outdir/Result/08.Comparison/Hcluster_tree");
	$queue->addcommond("mkdir -p $outdir/Result/08.Comparison/Pca");
	$queue->addcommond("mkdir -p $outdir/Result/08.Comparison/Pcoa");
	#$queue->addcommond("mkdir -p $outdir/Result/08.Comparison/Phylogeny_tree");
	#$queue->addcommond("cp $outdir/08.Comparison_diversity/phylogeny_tree/*.rep_aligned.tre $outdir/Result/Beta/Phylogeny_tree");
	#$queue->addcommond("cp $outdir/08.Comparison_diversity/phylogeny_tree/*.rep_aligned.fasta $outdir/Result/Beta/Phylogeny_tree");
    
	$queue->addcommond("cp -r $outdir/comparison/adonis/* $outdir/Result/08.Comparison/Adonis/");
	$queue->addcommond("cp -r $outdir/comparison/anosim/* $outdir/Result/08.Comparison/Anosim/");
	$queue->addcommond("cp -r $outdir/comparison/distance/* $outdir/Result/08.Comparison/Distance/");
	$queue->addcommond("cp -r $outdir/comparison/nmds/* $outdir/Result/08.Comparison/Nmds/");
	$queue->addcommond("cp -r $outdir/comparison/hcluster_tree/* $outdir/Result/08.Comparison/Hcluster_tree/");
	$queue->addcommond("cp -r $outdir/comparison/pca/*  $outdir/Result/08.Comparison/Pca/");
	$queue->addcommond("cp -r $outdir/comparison/pcoa/* $outdir/Result/08.Comparison/Pcoa/");
	$queue->addcommond("rm  $outdir/Result/08.Comparison/*/*/*/*r");

	$queue->addcommond("mkdir -p $outdir/Result/09.Difference");
	$queue->addcommond("mkdir -p $outdir/Result/09.Difference/LEfSe");
	$queue->addcommond("mkdir -p $outdir/Result/09.Difference/Wilcoxon");
	$queue->addcommond("cp -r $outdir/difference/lefse/*  $outdir/Result/09.Difference/LEfSe/");
	$queue->addcommond("cp -r $outdir/difference/classic/*  $outdir/Result/09.Difference/Wilcoxon");
	$queue->run();
	$queue->wait();
